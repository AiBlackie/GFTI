...
def load_data():
    ...
    # Calculate Trust Tax
    # Expected yield based on rating only (simplified model)
    # Base: 2.5% + (rating_num * 0.35%)
    df['Expected_Rating_Only'] = 2.5 + (df['SP_Numeric'] * 0.35)

    # Expected yield based on rating + GFTI (10bps per GFTI point)
    # Fill NaN GFTI with mean for calculation
    gfti_mean = df['GFTI_Score'].mean()
    df['GFTI_Score_Filled'] = df['GFTI_Score'].fillna(gfti_mean)
    df['Expected_Rating_GFTI'] = df['Expected_Rating_Only'] - (df['GFTI_Score_Filled'] * 0.10)

    # Trust Tax (actual vs expected with GFTI)
...
The Trust Tax measures the premium investors charge due to transparency concerns.

| Calculation | Value |
|-------------|-------|
| Actual Bond Yield | {bond_yield}% |
| Expected Yield (S&P Rating Only) | {country_data['Expected_Rating_Only']:.2f}% |
| Expected Yield (S&P + GFTI) | {country_data['Expected_Rating_GFTI']:.2f}% |
| **Trust Tax (bps)** | **{trust_tax} bps** |
| **Annual Cost on $1B Debt** | **${annual_cost}M** |

*Interpretation: {country} pays ${annual_cost}M more per year on every $1B of debt than its GFTI score would predict.*
...
❌ **Transparency gap** - Trust Tax of {trust_tax} bps costs ${annual_cost}M annually per $1B debt
"""
    else:
        report += f"""
...
| **4** | Reduce Trust Tax through improved transparency | 3-5 years |

### For Investors

| Consideration | Implication |
|---------------|-------------|
| **Risk premium** | {trust_tax} bps premium for transparency concerns |
...
This report is generated by the Global Financial Transparency Index (GFTI) using publicly available information. All findings are verifiable from original sources. The Trust Tax calculation uses the GFTI preliminary model: 10bps per GFTI point adjustment to expected yield.
...
with st.sidebar:
    ...
    filtered_df = filtered_df[
        (filtered_df['GFTI_Score'].fillna(50) >= min_gfti) &
        (filtered_df['GFTI_Score'].fillna(50) <= max_gfti) &
...
with col2:
    ...
    st.metric("Average Bond Yield", f"{avg_yield:.2f}%")

with col3:
    avg_trust_tax = filtered_df['Trust_Tax_bps'].mean()
    st.metric("Average Trust Tax", f"{avg_trust_tax:.0f} bps")

with col4:
    total_hidden = filtered_df['Annual_Cost_1B'].sum()
    st.metric("Annual Hidden Cost ($1B each)", f"${total_hidden:.1f}M")

st.markdown("---")

# ============================================================================
# GLOBAL MAP (Simplified - Table instead of map for now)
# ============================================================================
st.markdown('<div class="sub-header">🗺️ Global Transparency Overview</div>', unsafe_allow_html=True)
...
with col1:
    # Create scatter plot
    plot_data = filtered_df.dropna(subset=['GFTI_Score']).copy()
    ...
    # Add trend line
    trend_data = plot_data.dropna(subset=['GFTI_Score', 'Bond_Yield']).copy()
    if len(trend_data) > 1:
        x_vals = trend_data['GFTI_Score'].values
        y_vals = trend_data['Bond_Yield'].values
...
with col2:
    ...
    st.markdown("""
    - **Downward slope:** Higher GFTI = Lower yield
    - **Within each color:** Countries with better books pay less
    - **Outliers:** Countries that cross color boundaries represent opportunities

    **Your finding:** ~10bps per GFTI point
    """)

    # Calculate correlation
    corr_data = plot_data.dropna(subset=['GFTI_Score', 'Bond_Yield'])
    if len(corr_data) > 1:
        corr = corr_data['GFTI_Score'].corr(corr_data['Bond_Yield'])
        st.metric(
            "Correlation",
            f"{corr:.2f}",
...
# ============================================================================
# COUNTRY DEEP DIVE WITH REPORT DOWNLOAD
# ============================================================================
st.markdown('<div class="sub-header">🔍 Country Deep Dive & Report Download</div>', unsafe_allow_html=True)

selected_country = st.selectbox("Select a country to analyze:", filtered_df['Country'].tolist())
...
countries_to_compare = st.multiselect(
    ...
    default=['Barbados', 'Guyana', 'South Africa', 'United Kingdom'][:min(4, len(filtered_df))]
)
...
- Expected yield based on rating + GFTI
- The Trust Tax (basis points)
...
with col2:
    calc_rating = st.selectbox("S&P Rating", sorted(df['SP_Rating'].unique()))
with col3:
...
exp_gfti = exp_rating - (calc_gfti * 0.10)
trust_tax = exp_gfti - exp_rating  # Negative means GFTI improves expected yield
...
with col3:
    trust_tax_bps = trust_tax * 100 * -1  # Flip sign for intuitive display
    st.metric(
        "Trust Tax Impact",
        f"{trust_tax_bps:.0f} bps",
        "Lower is better" if trust_tax_bps > 0 else "Higher is worse"
    )

st.markdown(f"""
<div class="card" style="text-align: center;">
    <h3>Annual Cost on $1B Debt: ${(abs(trust_tax_bps)/100 * 10):.1f}M</h3>
    <p>Based on GFTI score of {calc_gfti} and S&P rating of {calc_rating}</p>
</div>
""", unsafe_allow_html=True)
...
<div style="text-align: center; color: #666; font-size: 0.9rem; padding: 20px;">
    ...
    <p>⚠️ Trust Tax calculation uses preliminary model: 10bps per GFTI point</p>
